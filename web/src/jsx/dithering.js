...
export function ordered(size, pixels, palette, blockLookup) {
	...
	for (let y = 0; y < height; y++) {
		for (let x = 0; x < width; x++) {
			...
			if (size === 2) {
				factor = BAYER_MATRIX_2X2[y % BAYER_MATRIX_2X2_SIZE][x % BAYER_MATRIX_2X2_SIZE];
			} else if (size === 4) {
				factor = BAYER_MATRIX_4X4[y % BAYER_MATRIX_4X4_SIZE][x % BAYER_MATRIX_4X4_SIZE];
			} else if (size === 8) {
				factor = BAYER_MATRIX_8X8[y % BAYER_MATRIX_8X8_SIZE][x % BAYER_MATRIX_8X8_SIZE];
			}
